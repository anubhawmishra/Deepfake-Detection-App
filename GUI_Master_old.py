...
image2 =Image.open('f8.jpg')
image2 =image2.resize((1500,790))
...
background_label.place(x=50, y=80) #, relwidth=1, relheight=1)
#
lbl = tk.Label(root, text="Deep Fake Detection", font=('times', 35,' bold '), width=65,height=1,bg="#152238",fg="white")
lbl.place(x=0, y=0)


frame_alpr = tk.LabelFrame(root, text=" --Image_Process-- ", width=920, height=120, bd=5, font=('times', 14, ' bold '),bg="brown")
frame_alpr.grid(row=0, column=0, sticky='nw')
frame_alpr.place(x=200, y=600)
...
def test_model_proc(fn):
    ...
    if fn!="":
        ...
        return A
    img = open(fn)    
    myfile = open("C:/Users/anubh/OneDrive/Desktop/deepfake/testing")
    if img == myfile:
          print(myfile)
...
def openimage():
    ...
    global fn
    print(fn)
    fileName = askopenfilename(initialdir='C:/Users/Sagar/Desktop/Image Foregery Detection 100% Code/testing', title='Select image for Aanalysis ',
                               filetypes=[("all files", "*.*")])
    IMAGE_SIZE=200
    imgpath = fileName
    fn = fileName
...
exit = tk.Button(frame_alpr, text="Next", command=window, width=15, height=1, font=('times', 15, ' bold '),bg="red",fg="white")
